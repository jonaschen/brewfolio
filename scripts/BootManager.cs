...
public partial class BootManager : Control
{
    ...
    [Export] private double SplashScreenDuration = 2.0; // 展示歡迎畫面的時長（秒）

    public override async void _Ready()
    {
        // --- 歡迎畫面邏輯 ---
        // 我們先等待一段時間，以便向使用者展示 Logo，留下美好的第一印象。
        await ToSignal(GetTree().CreateTimer(SplashScreenDuration), SceneTreeTimer.SignalName.Timeout);

        // --- 初始化服務 ---
        // 在歡迎畫面結束後，我們才取得需要的全域服務。
        _userProfileService = GetNode<UserProfileService>("/root/UserProfileService");
        _firebaseManager = GetNode<FirebaseManager>("/root/FirebaseManager");

        // --- 導航邏輯 ---
        // 現在，根據使用者狀態決定要前往的下一個場景。
