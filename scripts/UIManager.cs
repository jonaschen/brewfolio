...
public partial class UIManager : CanvasLayer
{
	...
	public override async void _Ready()
	{
		...
		_logoutButton.Text = "登出";
		// **FIXED**: Use GetViewport().GetVisibleRect() for Godot 4
		_logoutButton.Position = new Vector2(GetViewport().GetVisibleRect().Size.X - 120, 20);
		...
		UpdateRecipeList();
		// **FIXED**: Call the async version of the method
		await _recipeRepository.FetchRecipesFromServerAsync();
	}
	...
	{
		if (newState is IdleState) _statusLabel.Text = "待機中，請按開始。";
		else if (newState is GrindingState) _statusLabel.Text = "研磨中...";
		else if (newState is HeatingState) _statusLabel.Text = "加熱中...";
		else if (newState is BrewingState) _statusLabel.Text = "萃取中...";
	...
	private async void OnSaveButtonPressed()
	{
		...
		GD.Print("正在儲存配方...");
		// **FIXED**: Call the async version of the method
		await _recipeRepository.SaveRecipeAsync(_currentLogRecord);
	...
	private void OnLogoutButtonPressed()
	{
		...
		// NOTE: These methods need to be implemented in their respective services.
